...
//    cout << id << " " << tree[id][0] << "  " << tree[id][1]  << endl;
}

int lazy[4*300000+7];
void shift(int id,int l,int r)
{
//    cout << "before propagate " << id << " " << tree[id*2][0] << " " << tree[id*2+1][0] << endl;
    if(l!=r)
    {
        tree[id*2][0]+=lazy[id];
        tree[id*2][1]+=lazy[id];
...
//        lazy[id]=0;
//    }
//    cout << "after propagate " << id << " " << tree[id*2][0] << " " << tree[id*2+1][0] << endl;
}
...
pair<int,int> query(int id,int l,int r,int ql,int qr)
{
    ...
    if(l>qr ||  r<ql ) return make_pair(inf,-inf);
    if(l>=ql and r<=qr)
    {
//        cout << id << " " << l << " " << r << " *---> " << tree[id][0] << " " << tree[id][1] << endl;
...
//    cout << id << " theke return hocce " << min(a.fs,b.fs) << "  " << max(a.sc,b.sc) << endl;
...
int main()
{
    ...
    for(int i=0; i<n; i++)
        sf1(ara[i]);
    build(1,0,n-1);
//                cout << "after bulid " << endl;
//            for(int i=1;i<10;i++)
//            {
//                cout << i << " --> " << tree[i][0] << "  " << tree[i][1] << endl;
//            }
//    cout << tree[1][0] << " " << tree[1][1] << endl;
    while(m--)
    {
        int t;
        sf1(t);
